...
bool compile_file(void) {
    ...
    codegen->optimization_level = g_options.optimization_level;
    codegen->debug_info = g_options.debug_info;
    ...
    // Write ASTC bytecode file
    if (!codegen_write_to_file(codegen, g_options.output_file)) {
        fprintf(stderr, "Error: Failed to write ASTC file\n");
        codegen_destroy(codegen);
        semantic_destroy(semantic);
        ast_free(ast);
        parser_destroy(parser);
    ...
    printf("Generated bytecode size: %zu bytes\n", codegen->bytecode_size);
    if (g_options.verbose) {
        codegen_print_stats(codegen);
    }

    // Create legacy compatibility header (for compatibility with old tools)
    FILE* output = fopen(g_options.output_file, "a");
    if (output) {
        fprintf(output, "\n# ASTC Bytecode Metadata\n");
        fprintf(output, "# Generated from: %s\n", g_options.input_file);
        fprintf(output, "# Optimization level: %d\n", g_options.optimization_level);
        fprintf(output, "# Compilation phases: Lexical + Syntax + Semantic + CodeGen\n");
        fprintf(output, "# Bytecode size: %zu bytes\n", codegen->bytecode_size);
        fclose(output);
    }
