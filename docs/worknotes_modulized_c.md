...
##### T2.3.6: 拆分JIT编译器模块 ⏳ 待开始
- **目标**: 创建独立的JIT编译模块
- **预期时间**: 40分钟
- **具体任务**:
  - 提取行4729-5290的JIT编译代码
  - 包含缓存机制和多架构支持
  - 确保性能分析功能正常
- **输出**: `src/core/modules/pipeline_jit.c`

##### T2.3.7: 拆分虚拟机执行模块 ⏳ 待开始
- **目标**: 创建独立的ASTC虚拟机模块
- **预期时间**: 35分钟
- **具体任务**:
  - 提取行5484-6385的VM执行代码
...
##### T2.3.8: 创建核心协调模块 ⏳ 待开始
- **目标**: 创建新的pipeline_module.c作为核心协调层
- **预期时间**: 30分钟
- **具体任务**:
  - 保留原有的对外接口不变
  - 内部调用各个子模块功能
  - 实现模块间的数据传递和协调
- **输出**: 重构后的 `src/core/modules/pipeline_module.c`
...
**T2.3总预计时间**: 约5小时
**当前优先级**: 高 (pipeline_module过大影响开发效率)
**风险评估**: 中等 (API适配可能遇到问题，但有备份文件保障)
