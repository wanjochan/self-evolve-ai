...
int main() {
    ...
    // vm_execute_astc: 调用printf然后返回0
    // 这是一个简化的实现，实际应该解析和执行ASTC文件
    uint8_t vm_execute_code[] = {
        ...
        // 调用printf("Pipeline Module: vm_execute_astc called\n")
        // 注意：这里需要printf的地址，简化起见直接返回成功
    ...
    // 计算总代码大小
    size_t total_code_size = 1024; // 预留足够空间
    ...
    // 写入文件
    FILE* file = fopen("bin/pipeline_x64_64_test.native", "wb");
    if (!file) {
        printf("无法创建文件\n");
        free(code_section);
        return 1;
    ...
    printf("创建成功: bin/pipeline_x64_64_test.native\n");
