...
int compile_c_to_astc_direct(const char* c_file, const char* astc_file) {
    ...
    // 写入ASTC头部 (20 bytes)
    // magic[4] + version + flags + entry_point + source_size
    fwrite("ASTC", 1, 4, output);           // magic
    uint32_t version = 1;
    fwrite(&version, sizeof(uint32_t), 1, output);  // version
    uint32_t flags = 0;
    fwrite(&flags, sizeof(uint32_t), 1, output);    // flags
    uint32_t entry_point = 0;
    fwrite(&entry_point, sizeof(uint32_t), 1, output); // entry_point
    uint32_t src_size = (uint32_t)source_size;
    fwrite(&src_size, sizeof(uint32_t), 1, output); // source_size

    // 写入源代码
    fwrite(source_code, 1, src_size, output);

    // 写入字节码大小和数据
    uint32_t bytecode_size = 6;  // 简单的字节码: LOAD_CONST 42, RETURN
    fwrite(&bytecode_size, sizeof(uint32_t), 1, output);

    // 写入简单的字节码 (6 bytes)
    uint8_t bytecode[] = {
        0x41, 0x2A, 0x00, 0x00, 0x00,  // LOAD_CONST 42 (5 bytes)
        0xFF                            // EXIT (1 byte)
    };
    ...
    printf("c2astc: Binary ASTC file written to: %s\n", astc_file);
    printf("c2astc: File size: %zu bytes (header) + %u bytes (source) + %u bytes (bytecode)\n",
           20UL, src_size, bytecode_size);
